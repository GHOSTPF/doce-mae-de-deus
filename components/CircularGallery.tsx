...
const CircularGallery = React.forwardRef<HTMLDivElement, CircularGalleryProps>(
  // radius reduzido para aproximar as fotos | speed reduzido para o mínimo
  ({ items, className, radius = 50, autoRotateSpeed = 0.001, ...props }, ref) => {
              ...
              >
                <div className="relative w-full h-full rounded-lg shadow-2xl overflow-hidden group border border-border bg-card/70 dark:bg-card/30 backdrop-blur-lg">
                  ...
                  <div className="absolute bottom-0 left-0 w-full p-4 bg-gradient-to-t from-black/80 to-transparent text-white">
                    ...
                    <p className="text-[10px] mt-2 opacity-70">Photo by: {item.photo.by}</p>
                  </div>
                </div>
              </div>
            );
